...
interface Process {
  id?: string;                     // "processes:xxx" — auto-generated by backend; NEVER set on create
  domain_id?: string;              // owning domain record ID — auto-assigned; never set on create
...
- **Never include `id` when creating a new process** — the backend auto-generates it
