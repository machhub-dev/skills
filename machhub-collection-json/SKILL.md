...
#### User Reference Fields (createdBy, completedBy, acknowledgedBy, etc.)

When a field references a **user** (e.g., `createdBy`, `completedBy`, `acknowledgedBy`, `assignedTo`), set it as a `single` relation field with `relatedCollectionID` pointing to the users table — **not** a collection:
...
Key difference from regular collection relations: `Table` is `"users"` and `ID` is `"users"` (not `"collections"` with a specific collection ID).
...
- **Collection IDs**: Do **not** include a top-level `id` field on the collection object when creating — the backend auto-generates it. If you include one, the importer tries to preserve it but falls back to auto-generated if it conflicts
